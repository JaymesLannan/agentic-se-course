  ...
  questions: [
    {
      id: "m7-q1",
      type: "conceptual",
      text: "What is the relationship between test coverage and agent reliability? Explain specifically what an agent can and cannot do in a codebase with 15% test coverage vs one with 80% test coverage.",
      rubric: "Strong answer: 15% coverage — agent makes changes but has no feedback signal; declares success based on syntax/compilation not behaviour correctness; high risk of introducing bugs in untested paths; 80% — agent runs tests after changes, gets immediate failure signal, can iterate until tests pass, can confidently make changes in tested areas. Must explain the verification feedback loop. Weak answer says 'more tests is better'.",
      maxScore: 15,
      placeholder: "Explain what the agent can/cannot do at each coverage level...",
    },
    {
      id: "m7-q2",
      type: "conceptual",
    ...
    },
    {
      id: "m7-q3",
      type: "applied",
      text: "You're onboarding a new client whose codebase has: no CLAUDE.md, a src/helpers.js file with 1,400 lines, inconsistent naming (both getUserData() and fetchUser() exist for similar operations), 22% test coverage, and no clear separation between business logic and route handlers. Prioritise and plan your first 5 days of work to make this codebase agent-ready. Justify every priority decision.",
      rubric: "Strong answer prioritises: Day 1 — write CLAUDE.md with current state (even if messy) and most important conventions; Days 2-3 — identify the 20% of the codebase most likely to be touched by agents first and improve test coverage there; Day 4 — create an architecture document and separate the most critical mixed files (handlers from services); Day 5 — document generated files, add naming convention to CLAUDE.md. Justifies CLAUDE.md first (immediate agent guidance), targeted testing over full coverage (practical in 5 days). Weak answer says 'refactor everything'.",
      maxScore: 20,
      placeholder: "Prioritise the 5 days and justify every decision...",
    },
    {
      id: "m7-q4",
      type: "applied",
    ...
    },
    {
      id: "m7-q5",
      type: "hands_on",
      text: "Write a complete CLAUDE.md for the following codebase: A Python FastAPI application with a PostgreSQL database (SQLAlchemy ORM), Redis caching, Celery for background tasks, and pytest for testing. The architecture uses: routers (routes only), services (business logic), repositories (database access), schemas (Pydantic models). Tests live in tests/ mirroring the src/ structure. Generated files: alembic/versions/ (migrations). Key constraint: never call the database from routers or services directly.",
      rubric: "Strong answer covers all CLAUDE.md sections: project overview, tech stack, project structure with path descriptions, architecture rules (the direct DB constraint explicitly stated), key patterns with example file references, generated files section, commands (uvicorn, pytest, alembic), what to avoid. Should be specific enough that an agent reading it could navigate the codebase without any other context. Must include the architecture constraint prominently.",
      maxScore: 20,
      placeholder: "Write the complete CLAUDE.md...",
    },
    {
      id: "m7-q6",
      type: "hands_on",
    ...
    },
    {
      id: "m7-q7",
      type: "edge_case",
      text: "Six months after you've made a client's codebase agent-friendly, you return to find: the CLAUDE.md is outdated (references files that were renamed), the architecture rules are being violated more than before (the prohibition on direct DB calls is being ignored), and test coverage has dropped from 73% to 51% because the team has been shipping features faster without writing tests. What happened and how do you address it?",
      rubric: "Strong answer diagnoses: the system worked initially but lacked maintenance mechanisms; when the team moved fast with agents, they bypassed the guardrails the agent setup was meant to enforce; CLAUDE.md wasn't updated when files changed (suggest: add CLAUDE.md review to PR checklist); architecture violations suggest the team was using agents to bypass quality controls rather than maintain them (address with team culture and automated linting for layer violations); coverage drop is a predictable pattern when agents enable faster shipping without test discipline (fix: CI coverage threshold enforcement). Must address both technical and cultural fixes.",
      maxScore: 20,
      placeholder: "Diagnose what happened and address both technical and cultural aspects...",
    },
    {
      id: "m7-q8",
      type: "edge_case",
