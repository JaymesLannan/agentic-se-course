  ...
  questions: [
    {
      id: "m2-q1",
      type: "conceptual",
      text: "What is the fundamental difference between an automation and an agent?",
      options: [
        "Automations use cloud infrastructure while agents run locally on the user's machine",
        "Automations execute a hardcoded sequence of steps while agents have a model-controlled loop that determines what to do next",
        "Agents are faster than automations because they skip human review steps",
        "Automations can only handle one task at a time while agents can handle multiple concurrent workflows",
      ],
      correctAnswer: "Automations execute a hardcoded sequence of steps while agents have a model-controlled loop that determines what to do next",
      rubric: "Correct answer: automations execute a hardcoded sequence; agents have a model-controlled loop. The key distinction is who decides the next step — in automations it's the code, in agents it's the model. This difference drives every architectural and risk tradeoff between the two patterns.",
      maxScore: 10,
      placeholder: "",
    },
    {
    ...
    },
    {
      id: "m2-q3",
      type: "applied",
      text: "A client wants to build a system that automatically sends a welcome email, creates Slack and Jira accounts, and schedules onboarding meetings whenever a new employee is hired. How should this be classified and built?",
      options: [
        "Multi-agent system, because multiple external services need to be coordinated simultaneously",
        "Agent, because it involves multiple steps that require intelligent decision-making across systems",
        "Automation, because the steps are fixed and predictable with no dynamic decision-making required",
        "Chatbot, because it primarily involves sending messages and creating calendar events",
      ],
      correctAnswer: "Automation, because the steps are fixed and predictable with no dynamic decision-making required",
      rubric: "Correct answer: Automation. The steps are known in advance, execute in a fixed sequence, and require no model judgment about what to do next. Building this as an agent adds unnecessary complexity and risk without any benefit — agent architecture is only warranted when the path through a workflow cannot be determined ahead of time.",
      maxScore: 10,
      placeholder: "",
    },
    {
    ...
    },
    {
      id: "m2-q5",
      type: "hands_on",
      text: "A system monitors production logs, diagnoses errors when they appear, and then decides whether to auto-fix the issue or page the on-call engineer based on its assessment of severity. How should this be classified?",
      options: [
        "Automation, because it follows a fixed trigger-action pattern of monitor → diagnose → respond",
        "Chatbot, because it communicates with the on-call engineer via alerts and messages",
        "Agent, because the model must dynamically decide between two different actions based on its assessment",
        "Multi-agent, because monitoring, diagnosis, and response are three separate concerns requiring separate models",
      ],
      correctAnswer: "Agent, because the model must dynamically decide between two different actions based on its assessment",
      rubric: "Correct answer: Agent. The key factor is the dynamic decision — auto-fix vs page — which cannot be hardcoded. The model must assess severity and choose a path. That model-controlled branching is the defining characteristic of an agent. A pure automation could trigger alerts but couldn't make the judgment call.",
      maxScore: 10,
      placeholder: "",
    },
    {
    ...
    },
    {
      id: "m2-q7",
      type: "edge_case",
      text: "A client insists on building an agent instead of an automation because 'agents are more impressive for our board presentation.' What is the best way to handle this?",
      options: [
        "Build the agent as requested — the client knows their business needs better than you do",
        "Refuse to proceed until the client agrees to use the correct architecture",
        "Validate the client's goal, then find a path that serves both — such as building the right automation with an agent component where it genuinely adds value",
        "Agree to build the agent but document in writing that you advised against it to protect yourself",
      ],
      correctAnswer: "Validate the client's goal, then find a path that serves both — such as building the right automation with an agent component where it genuinely adds value",
      rubric: "Correct answer: validate the goal and find a path that serves both needs. The board presentation goal is legitimate. Refusing (option 2) damages the relationship. Blind capitulation (option 1) produces the wrong architecture. The skilled SE finds a solution that is technically sound AND meets the client's real need — which is demonstrating sophisticated AI capability to their board.",
      maxScore: 10,
      placeholder: "",
    },
    {
